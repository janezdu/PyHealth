...
class TestHALO(unittest.TestCase):
    """Test cases for the HALO synthetic-EHR generator."""
    ...
    def test_model_initialization(self):
        """Vocab sizes are derived from the processor; generation is unconditional."""
        self.assertIsInstance(self.model, HALO)
        ...
        self.assertEqual(self.model.config.code_vocab_size, proc_vocab)
        self.assertEqual(self.model.config.label_vocab_size, 0)
        self.assertEqual(self.model.config.total_vocab_size, proc_vocab + 3)
    ...
    def test_model_forward(self):
        ...
        # y_prob: (B, n_ctx - 1, total_vocab_size)
        self.assertEqual(ret["y_prob"].shape[0], 2)
        self.assertEqual(ret["y_prob"].shape[1], self.model.config.n_ctx - 1)
        self.assertEqual(
            ret["y_prob"].shape[2], self.model.config.total_vocab_size
        )
    ...
    def test_generate(self):
        ...
        synthetic = self.model.generate(num_samples=4, random_sampling=True)
